...
本文档为聚合sdk接入配置文档。用户可以参考AdvanceSdkDemo工程中的配置以及广告位接入代码进行开发。

目前聚合SDK支持管理的广告位类型为：

- 横幅广告位(Banner)： 广点通Banner2.0类型，穿山甲，Mercury

- 开屏广告位(Splash)：广点通，穿山甲，Mercury

- 插屏广告位（Interstitial)：广点通，穿山甲，Mercury

- 原生模板信息流广告位(NativeExpress): 广点通，穿山甲,Mercury
...
implementation(name: 'mercurysdk', ext: 'aar')
...
在res/xml目录下，新建广点通，Mercury的兼容配置file_path.xml,在文件中添加如下代码:
...
public class SplashActivity extends AppCompatActivity implements AdvanceSplashListener,WeakHandler.IHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        advanceSplash.setDefaultSdkSupplier(new SdkSupplier("12121x","1212xxxx","xxyyyxxyy",AdvanceConfig.SDK_TAG_MERCURY));
...
支持广点通，穿山甲,Mercury原生模板广告位。广告数据加载后需要转换为：GdtNativeAdExpressAdItem或者CsjNativeExpressAdItem，MercuryNativeExptess包装类进行操作，包装类包装了原始的广点通和穿山甲模板广告位API。
