...
|当前支持的版本号:| v3.4.1 |  v4.310.1180 | v3.4.1.2 |  内置v5.96| 内置v3.3.5.3| v3.4.4
|SDK ID 标识:| "1" |  "2"|  "3"|  "4"|  "5"| -
...
6. **策略最佳实践**说明
   ...
   本说明旨在引导各类APP进行正确的最优广告投放效果：
   
   一、 工具效果类APP：
   
   此类APP基本不存在CPT投放，建议 **使用缓存策略模式**，建议策略缓存时间>7天，缓存下加载策略仅**10ms左右**，非缓存平均**200ms左右**，缓存可以缩短单次广告请求整体耗时进而提升广告展示效果。
