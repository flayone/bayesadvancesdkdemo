...
4. SDK渠道平台信息：
	...
	|:-------------|:---------------|  :---------------|  :---------------| 
| Mercury|0.3M |v3.4.2 | "1" 
|广点通| 1.4M  |  v4.330.1200 | "2"| 
|穿山甲| 1.8M |v3.4.5.0|  "3"
| 百度|0.6M | v5.96|  "4"
|快手|1.1M |v3.3.5.3|  "5"
...
dependencies {
        ...
        //必须，Advance聚合SDK 引入。如果gradle引入失败，请先参考根目录gradle进行配置，如果依然无法引入，请联系我们使用本地aar引入。
        implementation "com.advsdk:advance:${advanceVersion}"
        //必须，MSA Android10以上设备唯一标识 OAID SDK 引入
        implementation 'com.advsdk:advance_oaid:1.0.25'
        //必须，mercury SDK 引入 包含了广告aar以及渠道适配代码
        implementation "com.advsdk:advance_mry:${advanceVersion}_342"
        //穿山甲、广点通、百度SDK 引入，包含了广告aar以及渠道适配代码(如果不希望使用带aar引入的advance渠道库，可以在版本号后面加上 _core 标识，这样就只会引入渠道适配代码，但是请注意，非advance推荐版本可能会出现广告加载异常，一定要充分测试)
        implementation "com.advsdk:advance_csj:${advanceVersion}_3450"
        implementation "com.advsdk:advance_gdt:${advanceVersion}_4330"
        implementation "com.advsdk:advance_bd:${advanceVersion}_596"
        //快手SDK 引入 仅包含了渠道适配代码
        implementation "com.advsdk:advance_ks:${advanceVersion}_3353_core"
        //快手SDK 引入 仅包含了快手广告aar
        implementation "com.advsdk:advance_kslib:${advanceVersion}_3353"
}
```


项目根目录build.gradle配置参考，注意jcenter()的位置：
...
- 需要注意如果广点通广告位使用的是否为**模板2.0**，2.0需要进行特殊的处理逻辑，且1.0和2.0互相不兼容
...
- 支持提前加载广告（注意控制时机，不可过早提前），但要注意，一次展示需对应一个广告实例，切忌复用广告实例。
- 需要注意如果广点通广告位使用的是否为**模板2.0**，1.0和2.0互相不兼容。
