...
<font size = "4">**版本号：v3.4.4**


## 1.集成须知

1. Advance 聚合SDK示例Demo	:
[点击下载最新Demo](http://www.bayescom.com/download/sdk/AdvanceSDK_Android_3.4.4.zip)
...
| 百度 |支持  |支持  |支持 ||| |支持 
	...
	<font size = "2"> **友情提示**：
	- 1.表格中不支持的渠道平台 or 广告位，可以通过下面 **7.自定义广告位** 来实现集成。
	- 2.除以下广告位：**百度Banner**，其他所有广告位均已支持并行加载
...
|当前支持的版本号:| v3.3.4 |  v4.310.1180 | v3.4.0.0 |  内置v5.96| 内置v3.3.5.3| v3.4.4
|SDK ID 标识:| "1" |  "2"|  "3"|  "4"|  "5"| -
...
SDK引入主要有两种方式，一是通过线上gradle依赖引入，二是通过本地aar包引入；如果gradle引入不成功可以联系我们提供本地aar来引入，以下为各个SDK的作用及推荐引入方式：
...
- Mercury SDK，平台广告渠道包，通过gradle引入（推荐）:
	
	```implementation 'com.advsdk:advance_mry:x.x.x'```
	
- 穿山甲SDK，头条穿山甲SDK广告渠道包，通过gradle引入（推荐）:
	
	```implementation 'com.advsdk:advance_csj:x.x.x'```

- 广点通SDK，腾讯优量汇SDK广告渠道包，通过gradle引入（推荐）:
	
	```implementation 'com.advsdk:advance_gdt:x.x.x'```
...
dependencies {
    ...
    //Advance聚合SDK 引入，gradle引入异常可以联系我们使用本地aar引入
    implementation 'com.advsdk:advance:3.4.4'
    //mercury SDK 引入
    implementation 'com.advsdk:advance_mry:3.4.4_334'
    //穿山甲SDK 引入
    implementation 'com.advsdk:advance_csj:3.4.4_3400_1'
    //广点通SDK 引入
    implementation 'com.advsdk:advance_gdt:3.4.4_4310'
    //百度SDK 引入
    implementation 'com.advsdk:advance_bd:3.4.4_596'
    //快手SDK 引入
    implementation 'com.advsdk:advance_ks:3.4.4_3353'
