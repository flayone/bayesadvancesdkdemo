...
| SDK渠道平台: | 	大小  | 兼容版本号|  SDK标识 ID |
|:-------------|:---------------|  :---------------|  :---------------| 
| 倍业|0.3M |v3.5.2 | "1" 
|优量汇(腾讯)| 1.4M  |  v4.431.1301 | "2"| 
|穿山甲| 1.8M |v4.1.0.5|  "3"
| 百度|0.6M | v9.18|  "4"
|快手|1.1M |v3.3.18.1|  "5"
...
参考APP项目目录下build.gradle配置：


项目根目录build.gradle配置参考，需要额外添加下方**s01.oss.sonatype**根域名的依赖地址：
<br/>**注意** ：如果同时依赖了阿里云，要放在阿里云的依赖前面，否则可能会出现部分依赖包找不到的提示。

```groovy
allprojects {
    repositories {
        //advance相关引用的maven依赖根目录地址，需配置在最前面，防止依赖找不到
