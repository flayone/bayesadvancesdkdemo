...
4. SDK渠道平台信息：
	...
	|:-------------|:---------------|  :---------------|  :---------------| 
| Mercury|0.3M |v3.4.6 | "1" 
|广点通| 1.4M  |  v4.373.1243 | "2"| 
|穿山甲| 1.8M |v3.8.0.3|  "3"
| 百度|0.6M | v9.131|  "4"
|快手|1.1M |v3.3.10.4|  "5"
...
dependencies {
    //引入Advance SDK
    def advanceVersion = "3.5.2"
    //必须，Advance聚合SDK 引入。如果gradle引入异常，请参考根目录gradle进行配置，如果依然无法引入，请联系我们使用本地aar引入。
    implementation "io.github.flayone:advance-core:${advanceVersion}_1"
    //必须，mercury SDK 引入 包含了广告aar以及渠道处理逻辑
    implementation "io.github.flayone:advance-mry:${advanceVersion}_346"
    //穿山甲、广点通、百度、快手SDK 引入，包含了广告aar以及渠道处理逻辑(如果APP本地引入了平台SDK的aar，不希望使用带aar引入的advance渠道库，可以在最后面加上 _core 标识，这样就不会产生aar引入冲突了，但是请注意，非advance推荐版本可能会出现广告加载异常，一定要充分测试)
    implementation "io.github.flayone:advance-gdt:${advanceVersion}_4373"
    implementation "io.github.flayone:advance-csj:${advanceVersion}_3803"
    implementation "io.github.flayone:advance-bd:${advanceVersion}_9131"
    implementation "io.github.flayone:advance-ks:${advanceVersion}_3310"
}
```
