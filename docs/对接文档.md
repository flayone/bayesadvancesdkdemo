...
3. 各个广告SDK平台聚合支持情况
 ...
 | 平台\广告位| 	开屏 |  激励视频 | 横幅| 插屏(弹窗) | 信息流 | 全屏视频 | Draw信息流| 自定义广告
|:------------- |:---------------|  :---------------|  :---------------|  :---------------|  :---------------|  :---------------|   :---------------|   :---------------|    
| Mercury |✅ |✅ |✅ |✅ |✅ | ❌|❌| ✅
| 广点通  |✅ |✅ |✅ |✅ |✅ |✅ |❌|✅
| 穿山甲|✅ |✅ |✅  |✅|✅|✅  |✅|✅
...
补充说明：如果您希望集成的SDK渠道不在上述列表中，比如**华为、OPPO、小米**等SDK渠道，建议使用**自定义SDK渠道**功能来实现，[点此了解详细说明](advance/sdk/android/customSDK.md)。
...
推荐使用gradle引入，如果gradle引入不成功可以联系我们提供本地aar来引入，引入参考**APP项目目录**以及**根目录**下build.gradle配置

**注意：** 渠道依赖包默认打包了各平台SDK的aar包，如果本地有引入相同SDK渠道aar请移除本地包，或者使用core渠道包。
<br/>项目根目录build.gradle配置参考，需要额外添加下方**s01.oss.sonatype**根域名的依赖地址：
<br/> 如果同时依赖了阿里云，要放在阿里云的依赖前面，否则可能会出现部分依赖包找不到的提示。
 

## 3.OAID说明

SDK内部已集成oaid获取方法，不再需要额外添加msa SDK来获取。
...
通过调用 `AdvanceSDK.setPrivacyController(AdvancePrivacyController controller)`方法，传入隐私控制器，可对部分隐私信息进行手动关闭或者直接赋值。

**其中`AdvancePrivacyController`中对应字段及含义，请参考下方说明了解：**
...
<details  <font color="#0000ff"> </font> 
<summary> <font color="#0000ff">查看更多可选配置信息 </font> </summary>

具体可参考AdvanceSDK和AdvanceConfig类文件中所有方法。

| 方法名| 	方法介绍 
|:------------- |:---------------| 
|AdvanceSDK.setPrivacyController(AdvancePrivacyController controller)|3.6.3 新增，设置隐私控制开关
|AdvanceSDK.setTanxImgLoader(ILoader loader) |3.6.0新增，tanx配置优化项，当glide不兼容时必填
|AdvanceSDK.setUseHttps(boolean useHttps)|3.5.4新增，设置是否使用https域名请求advanceSDK  
| AdvanceSDK.enableMD5UserInf(boolean md5) |3.5.4新增， 设置是否对传输的个人信息进行md5加密处理 
|AdvanceSDK.setCSJSplashButtonType(int splashButtonType) | 3.5.3新增，穿山甲特殊设置，用于控制开屏广告的点击区域，具体设置值及含义可参考：  https://www.pangle.cn/support/doc/611f0f0c1b039f004611e4da 
|AdvanceSDK.setCSJDownloadType(int downloadType) | 3.5.3新增，穿山甲特殊设置，用于控制下载APP前是否弹出二次确认弹窗(适用所有广告类型)。具体设置值及含义可参考：  https://www.pangle.cn/support/doc/611f0f0c1b039f004611e4da 
|  AdvanceSDK.setSplashPlusAutoClose(int time)  |3.5.2新增，设置开屏v+ 小窗口自动关闭时间，不设置使用默认各个渠道默认展示逻辑 
|AdvanceSDK.setLogLevel(AdvanceLogLevel logLevel) | 3.5.2新增，设置本地日志打印级别。默认DEFAULT，只打印核心日志 
| AdvanceSDK.setAppNameInCsj(String appName) |设置穿山甲后台中填写的应用名称 
| AdvanceConfig.getInstance().setKsAppName(String ksAppName)|设置在快手平台申请应用的应用名称 
| AdvanceConfig.getInstance().setNeedPermissionCheck(boolean needPermissionCheck)|穿山甲全局设置项 <br/> true 代表请求穿山甲广告前会进行权限检查和申请，默认false  
|AdvanceConfig.getInstance().setSupportMultiProcess(boolean supportMultiProcess)  |穿山甲全局设置项  <br/> 是否支持多进程，true支持，默认true  
|AdvanceConfig.getInstance().setCsjDirectDownloadNetworkType(int[] csjDirectDownloadNetworkType)  |穿山甲全局设置项 <br/> 设置穿山甲允许直接下载的网络状态集合，状态集合参考 TTAdConstant.NETWORK_STATE_XX  
|MercuryAD.setOAID(Context context, String oaid)|Mercury SDK 全局设置项  <br/> 设置Mercury SDK的OAID值，预留防止msa版本不一致导致获取不到OAID，导致填充低  
|MercuryAD.needPreLoadMaterial(boolean needPreLoadMaterial)  |Mercury SDK 设置是否允许预缓存素材，默认flase不预缓存 
...
|setShowInSingleActivity(boolean single)| **v3.5.2新增**，如果开屏页是fragment或者dialog实现，这里需要置为true。默认为false，代表开屏和首页为两个不同的activity
| setLogoLayout(@LayoutRes int logoLayout, int heightPX) | v4.1.2 新增 ， 开屏底部logo区域设置，使用layout资源引入，宽度全屏撑满，高度使用传入的高度像素值。当传入后，会在广告adContainer底部加上logo布局，其中MercurySDK会自适应选择是否显示logo布局，其他SDK将一直显示logo布局
|setOnlyMercuryUserLogo(boolean onlyMercuryLogoShow) | v4.1.2 新增 ，是否仅Mercury渠道使用logo配置，其他adn将使用全屏展示，默认false。希望网盟adn全屏展示得可以将此值置为true。
|以下为穿山甲 SDK 渠道特有设置
...
|jumpToMain()| v3.5.2新增，跳转到首页方法，直接在此回调方法中进行跳转首页行为
...
### 6.3插屏(弹窗)广告(AdvanceInterstitial)
...
|loadStrategy()| 加载广告策略，根据返回的渠道顺序进行请求并展示广告。| 必须
|setAdContainer(ViewGroup container) |  v3.5.3新增 设置用来展示广告的父布局 | 必须
| setVideoMute(boolean mute) | v3.5.3新增 设置视频类广告是否静音，默认true，仅对部分渠道生效 | 可选
|enableStrategyCache(boolean enableCache) | 是否开启策略缓存。注意这里缓存的是SDK的调度策略，而非缓存的广告信息。不设置默认关闭   |  可选，具体参考**1.6 最佳实践说明**
...
信息流子View相关API---已废弃（AdvanceNativeExpressAdItem）
...
|setMute(boolean mute) |v3.6.4 新增，设置静音选项，仅对优量汇、tanx生效
</details>
...
* 基础全屏视频实现类 **AdvanceFullScreenItem** （已废弃）
...
```java
//初始化广告
advanceDraw = new AdvanceDraw(mActivity, "聚合draw视频信息流广告位id");
//必须：设置广告承载布局
advanceDraw.setAdContainer(adContainer);
//推荐：核心事件监听回调
advanceDraw.setAdListener(this);
//必须：请求策略并请求广告
...
* 请求策略、加载广告 **AdvanceDraw**

| 方法名| 	方法介绍|  是否必须
|:------------- |:---------------|  :---------------|  
| AdvanceDraw(Activity activity,String adspotId) | 广告初始化方法，adspotId为聚合广告位id | 必须
|setAdListener(AdvanceDrawListener listener) | 设置各个事件的回调监听器，具体回调含义参考下方AdvanceDrawListener 说明| 可选
|loadStrategy()| 加载广告策略，根据返回的渠道顺序进行请求并展示广告。| 必须
|enableStrategyCache(boolean enableCache) | 是否开启策略缓存。注意这里缓存的是SDK的调度策略，而非缓存的广告信息。不设置默认关闭   |  可选，具体参考**1.6 最佳实践说明**
|setAdContainer(ViewGroup adContainer) | 设置广告展示的承载父布局 | 必须

*  事件回调 **AdvanceDrawListener**

| 方法名| 	方法介绍|  
|:------------- |:---------------|  
|onAdLoaded() |成功获取广告信息回调，广告准备就绪
|onAdFailed(AdvanceError advanceError)  | 获取广告信息失败回调
|onAdShow() |  视频页面展示成功回调
|onAdClicked()|	广告被点击回调
|onSdkSelected(String sdkID) | 当前选中的SDK渠道，sdkID对应 **1.4 SDK渠道平台信息**中的SDK ID
...
自定义插屏(弹窗)广告和自定义自渲染原理类似，具体代码请参考最新demo中的`CustomInterstitialActivity`类文件。
...
Advance聚合SDK内部已经集成支持了部分SDK渠道（广点通、穿山甲等SDK），但是对于**未集成的SDK渠道**，开发者想要集成使用就会变得比较困难。而自定义SDK渠道就是让用户**自由集成所有SDK渠道，并统一管理**的，在保证广点通、穿山甲等主流SDK集成的情况下，可以快速、自由地接入各种第三方SDK，并在Advance的后台统一管理。具体流程请参考下文内容，示例代码可以参考demo示例工程。


### 2.自定义流程

我们以自定义小米广告SDK开屏广告为例，来自定义实现Advance SDK支持加载“小米SDK”广告。

#### step1：集成目标SDK

- 首先在小米开放平台官网下载最新版本的SDK包：mimo-ad-sdk.aar，将此aar放在项目lib目录下，并在项目build.gradle中引入aar文件。
- 按照集成文档中要求，添加权限配置、AndroidManifest.xml配置、混淆配置等必要性配置。


#### step2：自定义该SDK渠道的适配adapter

新建`XiaoMiSplashAdapter`类文件，继承于`AdvanceSplashCustomAdapter`类，实现构建函数和抽象方法，并添加**SDK初始化、广告位初始化、加载、回调**等核心代码。

注意：渠道的**媒体id**和**广告位id**建议通过封装好的方法`getAppID`和`getPosID`来获取并填入渠道设置方法中。
...
public class XiaoMiSplashAdapter extends AdvanceSplashCustomAdapter {
    ...
    @Override
    protected void adReady() {//广告就绪（不区分串、并行）
...
#### step3：添加自定义adapter至聚合广告位

初始化`AdvanceSplash`开屏广告后，调用`addCustomSupplier`方法将自定义的adapter添加至开屏广告位的adapter队列中。代表现在开屏是支持了小米SDK渠道的处理，只要在后台策略配置中，添加上小米SDK渠道，并填入id信息即可。
...
* 1.确保SDK渠道资源引入正确，以及广告位id可正常使用状态。

* 2.注意该SDK渠道的广告请求和展示方法是否分离，只有分离时才可以支持**并行加载**，比如小米的开屏广告的请求和展示方法为`loadAndShow`统一调用，那么就不支持并行加载。另外如果支持并行加载，需要在`paraLoadAd`和`orderLoadAd` 方法中执行广告请求代码，在`showAd`中执行广告展示代码。
* 3.不同广告位自定义`adapter`需继承不同的基类`CustomAdapter`，比如开屏需要继承于`AdvanceSplashCustomAdapter` ，插屏需要继承于`AdvanceInterstitialCustomAdapter`。如果不太清楚不同广告位的处理方式，可以参考聚合SDK中已支持的SDK渠道adapter代码，比如`MercuryInterstitialAdapter`中代码进行广告加载、展示、上报等逻辑处理。
* 4.对于自定义的adapter中，流程节点的回调反馈是十分重要的，尤其是`handleSucceed`，`handleShow`，`handleClick`，`handleFailed` 这4个代表了广告成功、广告展示、广告点击、广告失败的事件跟踪一定要正确调用。
